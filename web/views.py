...
def get_docs1(cur_page, item_per_page, s_code):
    ...
    doc_count = Qikan.objects.count()
    page_count = doc_count // item_per_page  # 计算出一共多少页(
    field = ",".join(field_config.keys())
    offset_item = (cur_page-1) * item_per_page
    sql = "select " + field + " from web_qikan where (s_code &" + str(s_code) + ") = "+str(s_code)+" limit "+str(item_per_page)+" offset "+ str(offset_item)
    ...
    # paginator = Paginator(all_docs, item_per_page)  # Show 25 contacts per page
    # try:
    #     qikan_docs = paginator.page(cur_page)
    # except PageNotAnInteger:
    #     # If page is not an integer, deliver first page.
    #     qikan_docs = paginator.page(1)
    # except EmptyPage:
    #     # If page is out of range (e.g. 9999), deliver last page of results.
    #     qikan_docs = paginator.page(paginator.num_pages)
